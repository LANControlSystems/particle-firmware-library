...
#include "memfault-firmware-sdk/components/include/memfault/core/compact_log_compile_time_checks.h"
#include "memfault-firmware-sdk/components/include/memfault/core/compact_log_helpers.h"
#include "memfault-firmware-sdk/components/include/memfault/core/event_storage.h"
#include "memfault-firmware-sdk/components/include/memfault/core/trace_event_impl.h"
#include "memfault-firmware-sdk/components/include/memfault/core/trace_reason_user.h"
...
#define MEMFAULT_TRACE_EVENT(reason)                                               \
  do {                                                                             \
    ...
    memfault_trace_event_capture(MEMFAULT_TRACE_REASON(reason), mflt_pc, mflt_lr); \
  } while (0)
...
#define MEMFAULT_TRACE_EVENT_WITH_STATUS(reason, status_code)                                 \
  do {                                                                                        \
    ...
    void *mflt_lr;                                                                            \
    MEMFAULT_GET_LR(mflt_lr);                                                                 \
    memfault_trace_event_with_status_capture(MEMFAULT_TRACE_REASON(reason), mflt_pc, mflt_lr, \
                                             status_code);                                    \
  } while (0)
  ...
  #define MEMFAULT_TRACE_EVENT_WITH_LOG(reason, ...)                                         \
    do {                                                                                     \
      ...
      void *mflt_lr;                                                                         \
      MEMFAULT_GET_LR(mflt_lr);                                                              \
      memfault_trace_event_with_log_capture(MEMFAULT_TRACE_REASON(reason), mflt_pc, mflt_lr, \
                                            __VA_ARGS__);                                    \
    } while (0)
  ...
  #define MEMFAULT_TRACE_EVENT_WITH_LOG(reason, format, ...)                       \
    do {                                                                           \
      ...
      memfault_trace_event_with_compact_log_capture(                               \
        MEMFAULT_TRACE_REASON(reason), lr, MEMFAULT_LOG_FMT_ELF_SECTION_ENTRY_PTR, \
