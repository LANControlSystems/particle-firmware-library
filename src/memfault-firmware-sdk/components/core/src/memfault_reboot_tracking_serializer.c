...
static bool prv_serialize_reboot_info(sMemfaultCborEncoder *e, const sMfltResetReasonInfo *info) {
  const size_t extra_event_info_pairs =
    1 /* coredump_saved */ + ((info->reset_reason_reg0 != 0) ? 1 : 0);
  ...
  if (success && info->reset_reason_reg0) {
    success = memfault_serializer_helper_encode_uint32_kv_pair(
      e, kMemfaultTraceInfoEventKey_McuReasonRegister, info->reset_reason_reg0);
  }
...
int memfault_reboot_tracking_collect_reset_info(const sMemfaultEventStorageImpl *impl) {
  ...
  sMemfaultCborEncoder encoder = { 0 };
  const bool success =
    memfault_serializer_helper_encode_to_storage(&encoder, impl, prv_encode_cb, &info);
