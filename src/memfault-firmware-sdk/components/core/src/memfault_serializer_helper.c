...
static bool prv_encode_event_key_string_pair(sMemfaultCborEncoder *encoder, eMemfaultEventKey key,
                                             const char *value) {
...
static bool prv_encode_device_version_info(sMemfaultCborEncoder *e) {
  ...
  if (!prv_encode_event_key_string_pair(e, kMemfaultEventKey_SoftwareVersion,
                                        info.software_version)) {
    return false;
  }

  if (!prv_encode_event_key_string_pair(e, kMemfaultEventKey_HardwareVersion,
...
bool memfault_serializer_helper_encode_uint32_kv_pair(sMemfaultCborEncoder *encoder, uint32_t key,
                                                      uint32_t value) {
  return memfault_cbor_encode_unsigned_integer(encoder, key) &&
         memfault_cbor_encode_unsigned_integer(encoder, value);
}

bool memfault_serializer_helper_encode_int32_kv_pair(sMemfaultCborEncoder *encoder, uint32_t key,
                                                     int32_t value) {
  return memfault_cbor_encode_unsigned_integer(encoder, key) &&
         memfault_cbor_encode_signed_integer(encoder, value);
}

bool memfault_serializer_helper_encode_byte_string_kv_pair(sMemfaultCborEncoder *encoder,
                                                           uint32_t key, const void *buf,
                                                           size_t buf_len) {
  return memfault_cbor_encode_unsigned_integer(encoder, key) &&
         memfault_cbor_encode_byte_string(encoder, buf, buf_len);
}

static bool prv_encode_event_key_uint32_pair(sMemfaultCborEncoder *encoder, eMemfaultEventKey key,
                                             uint32_t value) {
  return memfault_cbor_encode_unsigned_integer(encoder, key) &&
         memfault_cbor_encode_unsigned_integer(encoder, value);
}
...
bool memfault_serializer_helper_encode_metadata_with_time(sMemfaultCborEncoder *encoder,
  ...
  const bool unix_timestamp_available =
    (time != NULL) && (time->type == kMemfaultCurrentTimeType_UnixEpochTimeSec);
  ...
  MEMFAULT_STATIC_ASSERT(
    MEMFAULT_EVENT_INCLUDED_BUILD_ID_SIZE_BYTES >= 5 &&
  ...
  if (has_build_id && !memfault_serializer_helper_encode_byte_string_kv_pair(
                        encoder, kMemfaultEventKey_BuildId, info.build_id,
                        MEMFAULT_EVENT_INCLUDED_BUILD_ID_SIZE_BYTES)) {
    return false;
  }
#endif

  return !unix_timestamp_available ||
         prv_encode_event_key_uint32_pair(encoder, kMemfaultEventKey_CapturedDateUnixTimestamp,
...
bool memfault_serializer_helper_encode_trace_event(sMemfaultCborEncoder *e,
  ...
  const size_t num_entries = 1 /* reason */ + ((info->pc != 0) ? 1 : 0) +
                             ((info->lr != 0) ? 1 : 0) + info->extra_event_info_pairs;
  ...
  if (!memfault_serializer_helper_encode_uint32_kv_pair(e, info->reason_key, info->reason_value)) {
    return false;
  }
  ...
  if (info->pc) {
    success = memfault_serializer_helper_encode_uint32_kv_pair(
      e, kMemfaultTraceInfoEventKey_ProgramCounter, info->pc);
  }

  if (success && info->lr) {
    success = memfault_serializer_helper_encode_uint32_kv_pair(
      e, kMemfaultTraceInfoEventKey_LinkRegister, info->lr);
  }
...
static void prv_encoder_write_cb(void *ctx, MEMFAULT_UNUSED uint32_t offset, const void *buf,
                                 size_t buf_len) {
  const sMemfaultEventStorageImpl *storage_impl =
    ((sMemfaultSerializerHelperEncoderCtx *)ctx)->storage_impl;
  storage_impl->append_data_cb(buf, buf_len);
}

bool memfault_serializer_helper_encode_to_storage(
  sMemfaultCborEncoder *encoder, const sMemfaultEventStorageImpl *storage_impl,
  ...
  if (!success) {
    ...
    MEMFAULT_LOG_INFO("Event saved successfully after %d drops", (int)s_num_storage_drops);
...
size_t memfault_serializer_helper_compute_size(
  sMemfaultCborEncoder *encoder, MemfaultSerializerHelperEncodeCallback encode_callback,
  void *ctx) {
